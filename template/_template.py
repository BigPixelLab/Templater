...
class ParsingScope:
    ...
    SPECIAL_ATTRIBUTES = {DISPLAY_ATTRIBUTE, DUPLICATE_ATTRIBUTE}
    """ Аттрибуты, которые необходимо игнорировать при сборке аргументов для обработчика """

    REMAINING_ARGUMENT = '__rem'
    """ Аргумент в который будут переданы все указанные в теге, но не объявленные в обработчике атрибуты """

    IGNORE_ANNOTATIONS = ['tag', 'return']
    ...
    def __arguments__(
        ...
        for attribute, value in attributes.items():
            if attribute in self.SPECIAL_ATTRIBUTES:
                continue
    ...
    def _register(self, func, name: str = None, override: bool = False,
        ...
        for skip in self.IGNORE_ANNOTATIONS:
